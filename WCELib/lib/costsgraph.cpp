...
CostsGraph::CostsGraph(int size, int m, int64_t *starts, int64_t *ends, int64_t *weights, double th)  : _size(size), _threshold(th), forbidden(std::numeric_limits<double>::max() * -1), permanent(std::numeric_limits<double>::max()),  _cost_matrix(size, forbidden)
{
    for (int i = 0; i < m; i++) {
        _cost_matrix.pos(starts[i], ends[i]) = weights[i];
    }
