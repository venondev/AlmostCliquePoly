...
class graph_io {
 public:
    ...
    static std::shared_ptr<Graph> createGraphFromEdgeList(int64_t* starts, int64_t* ends, int64_t* weights, int n, int m) {
        std::shared_ptr<Graph> G = std::make_shared<Graph>();

        G->start_construction(n, m * 2);
        ...
        for (NodeID i = 0; i < m; i++ ) {
            G->new_edge(starts[i], ends[i], weights[i]);
        }
    ...
    template <class Graph = graph_access>
    static std::shared_ptr<Graph> readGraphCM(std::string file) {
        std::shared_ptr<Graph> G = std::make_shared<Graph>();
        std::string line;
...
//        G->number
        ...
        while (std::getline(instream, line)) {
            ...
            std::stringstream s;
            s.str(line);
            uint64_t j = nodeCounter + 1;
            while(std::getline(s, segment, '\t')) {
                double d = std::stod(segment);

                if (d > 0) {
                    EdgeWeight edge_weight = int(d);
                    edges.push_back(std::make_tuple(nodeCounter, j, edge_weight));
                }
        ...
        for (const auto& value : edges) {
            NodeID start = std::get<0>(value);
            NodeID end = std::get<1>(value);
            EdgeWeight weight = std::get<2>(value);

            G->new_edge(start, end, weight);
        }
